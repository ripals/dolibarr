...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_PMLogic.modules.php
 *	\ingroup    propale
 *	\brief      Fichier de la classe permettant de generer les propales au modele PM Logic
 */
...
/**
 *	Class to generate PDF proposal PMLogic
 */
class pdf_PMLogic extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "PMLogic";
		$this->description = $langs->trans('PM Logic Template');
			...
			if (file_exists($dir))
			{
	           ...
	           if (class_exists('TCPDF'))
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$pdf->setPageOrientation('', 1, $heightforfooter+$heightforfreetext+$heightforsignature+$heightforinfotot+$heightfornote);	// The only function to edit the bottom margin of current page to set it.
					$pageposbefore=$pdf->getPage();

					$showpricebeforepagebreak=1;
					$posYAfterImage=1;
					$posYAfterDescription=1;
					
					// We start with Photo of product line
					if (isset($imglinesize['width']) && isset($imglinesize['height']) && ($curY + $imglinesize['height']) > ($this->page_hauteur-($heightforfooter+$heightforfreetext+$heightforsignature+$heightforinfotot+$heightfornote)))	// If photo too high, we moved completely on new page
					{
						$pdf->AddPage('','',true);
						if (! empty($tplidx)) $pdf->useTemplate($tplidx);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						if ($posyafter > ($this->page_hauteur - ($heightforfooter+$heightforfreetext+$heightforsignature+$heightforinfotot+$heightfornote)))	// There is no space left for total+free text
						{
                    ...
                    if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
                    {
                        ...
                        $pdf->MultiCell($this->posxdesc-$this->posxpos-0.8, 3, $i+1, 0, 'C');
                    }
                    //##########################################################
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
					{
						...
						$pdf->MultiCell($this->posxlocaltax1-$this->posxtva-0.8, 3, $vat_rate, 0, 'R');
					}
					...
					{
						$vat_rate = pdf_getlinevatrate($object, $i, $outputlangs, $hidedetails);
						$pdf->SetXY($this->posxlocaltax1, $curY);
				...
				if ($pagenb == 1)
				{
					$this->_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter - $heightfornote, 0, $outputlangs, 0, 0, $object->multicurrency_code);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter + 1;
				}
				else
				{
					$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter - $heightfornote, 0, $outputlangs, 1, 0, $object->multicurrency_code);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter - $heightfornote + 1;
				}
				...
				// Affiche notes
				    ...
				    if ($notetoshow)
				    {
		                ...
		                // Rect prend une longueur en 3eme param
		                //$pdf->SetDrawColor(192,192,192);
		                //$pdf->Rect($this->marge_gauche, $tab_top-1, $this->page_largeur-$this->marge_gauche-$this->marge_droite, $heightnotetoshow);
		                //$pdf->MultiCell($this->posxpos-$this->posxdesc-0.8, $heightfornote, $print, 0, 'J');
                        $tab_top = $nexY+6;
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
        ...
        //$pdf->line($this->posxpos-1, $tab_top, $this->posxpos-1, $tab_top + $tab_height);	// line prend une position y en 2eme param et 4eme param
        if (empty($hidetop))
        {
            $pdf->line($this->marge_gauche, $tab_top+10, $this->page_largeur-$this->marge_droite, $tab_top+10);
            $pdf->SetXY($this->posxpos-1, $tab_top+1);
            $pdf->MultiCell(10,2, $outputlangs->transnoentities("Sr. No."),'','C');
        }  
		...
		{
			$pdf->line($this->posxdesc-1, $tab_top, $this->posxdesc-1, $tab_top + $tab_height);	// line prend une position y en 2eme param et 4eme param
            if (empty($hidetop))
            {
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(25,2, $outputlangs->transnoentities("TotalHT"),'','C');
		}
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
            ...
            $pdf->MultiCell(80, 4, 'GSTIN: 24AOVPS0254H1ZN', 0, 'L');
